...
inline void Manifold <SPHERE, 3>::tangent_plane_base(cref_type x, tm_base_ref_type result){
    ...
    result.col(0) = value_type(0, x.coeff(2), -x.coeff(1)) * (1-c) + value_type(x.coeff(2), 0, -x.coeff(0)) * c;
    result.col(0).normalize();
    result.col(1) = x.cross(result.col(0));
