...
static void arrowedLine(cv::Mat img, cv::Point pt1, cv::Point pt2, const cv::Scalar& color,
...
int main(int argc, const char *argv[])
{
	...
	vpp::pixel_wise(angle_with_border.domain(), BN) | [&] (const auto& d, const auto& n){
	    if(d(0)%10==0 && d(1)%10==0){
		double avg = 0;
		...
		cv::Point source(d(0),d(1));
		cv::Point target(d(0) + length * std::cos(avg), d(1) + length * std::sin(avg));
		arrowedLine(original, source, target, cv::Scalar( 0, 255, 0 ) );
	    }
	};
