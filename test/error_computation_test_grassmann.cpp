...
int main(int argc, const char *argv[])
{
	...
	typedef Manifold< GRASSMANN, 3, 1> mf_t;
	...
	std::string statfile_name, solfile_name, noisefile_name;
	
	std::stringstream sstream;

	sstream << fname << "_" << ny << "x" << nx;
	statfile_name = sstream.str() + "Gr_IRLSstats.csv";
	noisefile_name = sstream.str() + "Gr_noise.csv";
	solfile_name = sstream.str() + "Gr_sol.csv";
	
	solution.output_matval_img(noisefile_name.c_str());
...
//	solution.readMatrixDataFromCSV(argv[2], nx, ny);
	...
	int prpt_step_ = 1;
	int max_prpt_steps_ = 250;
	max_runtime_ = 10000;

	statfile_name = sstream.str() + "Gr_PRPTstats.csv";
