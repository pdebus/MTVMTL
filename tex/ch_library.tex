...
This is illustrated by the following short listing \ref{code:pixelwise_demo}:\\
...
Then an arithmatic operation is simultaneously applied to all variables in the register such that theoretically the amount of floating point operations is multiplied by the number
...
For the TV Minimizer class it makes sense to consider the IRLS and proximal point implementation separately. 
\cppinline
\begin{lstlisting}
//Primary Template 
...
\subsubsection{DT-MRI data denoising and visualization} % (fold)
\label{ssub:dti_tut}
