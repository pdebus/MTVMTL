...
Since the core parts of the implementation are originally based on a matlab prototype by Sprecher \cite{SprecherIRLS}, \cite{mt} and \cite{bt}, one of the most important
...
One reason for the change is improved readibility and maintainability of the code since tensor product usualluay tend to become very convoluted,
...
in their local tangent space basis represention, such that the degrees of freedom correspond the intrinsic manifold dimension not the dimension of the embedding space. This also reduces
...
existing manifold functionalting makes a modification of all \texttt{switch} cases necessary. There is no single point of change but many source files need to edited.\\
...
manifold class which has a specializaiton for every supported manifold type and due to the fact that the functions implemented in those class specializations are generally
just functions of one or two elements of the manifold, they could also be used in other projects which requiere the same functionality.\\
...
using the so-called \textit{pixel-wise kernels} of the VPP library, which makes it posible to map an arbitrary function on all pixels of a set of image containers: The function is
...
Then an arithmatic operation is simultaneously applied to all variables in the register (see figure \ref{fig:simd}) such that theoretically the amount of floating point operations is multiplied by the number
...
In order to really achieve this speedup the data must be aligned in memory which means the adress of pixels in memory must always be multiple of the SIMD register size. Fortunately,
that issue is handeld by the VPP and Eigen libraries enabling the compiler to perform the necessary vectorization optimizations.
...
The corresponding Matlab language construct is an anonymous function or funtion handle, usually defined using the \texttt{\@} operator. The following listing
...
an leads to code that is acutally equivalent to manually defining a function with the desired number of arguments and consequently there is no additional runtime effort.\\
...
So far, the manifold classes contains functionality necessary for TV minimization using either the IRLS or proximal point algorithm and furthermore some addtional operations
...
inline static void convex_combination(cref_type x, cref_type y, double t, ref_type result);
	...
	The parameters of the expontial here are not the manifolds own typedefs but the base class of all Eigen matrix data types. The reason for using this construction is that the function
	can also be called with composite expressions (e.g. $XY+Z$) without a temporary copy. Most of the other functions are usually called with atomic expresion only, hence there is no
...
inline static void karcher_mean(V& x, const Args&... args);
	...
	Creating the list for the first version eventuallys requires copying and is consequently slower but has an overloaded version which allows to compute a wighted Karcher mean
...
and edge weight information. For storage, the n-dimensional VPP \cite{vpp_paper} image container is used.\\
...
Since the memory management of the container is based on \texttt{std::shared\_pointer} it is also very easy to effciently access subimages or slices of an image without any copies.\\
...
comma-separated and rowwise flattened, such that each line of the input file contains exactly one pixel. The order of the list is also rowwise for 2D or slice- than rowwise for 3D images,
respectively. \\ 
...
The second template parameters \texttt{disc} determines whether the isotropic or the anisotropic version is to be used. Please not that for thr proximal point algorithm only
anistropic is available. Finally, the last parameter speciefies the dimensionality of the data. \\
...
\end{lstlisting}
As in the previous cases we have to provide fully specialized manifold, data and also functional types. Again, the last parameter speciefies the dimension of the data. Of the
...
Those can visualized by ellipsoids using three degerees of freedom for their orientation in space and the remaining three for length of its semi-axis.\\
...
\begin{figure}[h!]
    ...
    \caption[3D SPD(3) Volume Visualization of a helix]{Example of the 3D data Visulization of SPD(3) images from different viewpoints.
	\label{fig:helix}
    }
\end{figure}
...
Plasticity is created by setting the alpha channel of each displayed voxel to the intesity value of the corresponding data voxel, such that dark areas are more transparent.
...
\begin{itemize}
    ...
    \item SuiteSparse ($\geq 4.2.1$), faster parallel sparse solver for the linear sytem in the IRLS algorithm
\end{itemize}
...
For the sake of compactness and readabillity this should be done using \texttt{typedef}s. In the next step one can then
...
As a first example we show the denoising of a simple color picture usign the IRLS minimizer. In a first step the necessary classes need to be included. For the sake of shortening the 
...
Finally we choose the minimizer we want to use, in this case IRLS, and pass functional, manifold and data types as template parameters. The OMP paramater is not fully implemented yet 
...
will be included in the full listing in the appedix \ref{ap:example_listing}. This time, we start by also obtaining some of the minimization parameters from the command line:\\
...
by the \texttt{threshold} variable and then replace every RGB pixel by the mean of its three color components (those pixels are basically grat scale then).\\
