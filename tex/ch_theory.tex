...
\end{equation}
and the Riemannian distance can consequently be defined as
\begin{equation}
...
This rather general definition is used by \cite{Absil2009} wheras for our
...
proximal mappings. The latter is used by the so-called proximal point algorithm which will be shortly summarized in the next section before proceeding to
...
Let $M$ be a Riemannian manifold and $\Omega$ as defined in \label{sub:Discretization}. Due to the square root involved in the isotropic case,
...
\label{sub:IRLS}
The IRLS approach to dealing with non-differentiable terms in the functional is by adding addtional terms for regularization. In the continuous (and isotropic) case that means
\begin{equation}
    \label{eq:regulzarized_tv}
    TV_{\epsilon}=\int_{\Omega}\omega_{\epsilon}|\nabla u|^2 =\int_{\Omega}\frac{|\nabla u|^2}{\sqrt{|\nabla u|^2+\epsilon^2}}
\end{equation}
for a small $\epsilon>0$. In this form, the functional becomes differentiable and (\ref{eq:regulzarized_tv}) can be interpreted as weighted $L^2$ norm.
...
\end{align}
while the anisotropic steps are
\begin{align}
...
derivatives belonging to same gradient component ($x,y$ or $z$) populate subdiagonals of equal distance to the main diagonal.
...
of the squared distance functions, or tangent space mappings like the Hessian using only $\operatorname{dim} M$ coefficients. We now have $D=\operatorname{dim}M (XYZ)$ such that the 
...
\label{sub:Euclidian}
The space in question is just $\mathbb{R}^n$, hence trivially a manifold and naturally a vector space such that all expression can be calculated using basic
multivariable calculus. The exponential and logarithm mappings are not be understood in the sense of $e^x$ but according to \ref{sub:RiemannianDistance} such that
...
\begin{equation}
    ...
    \end{cases}.
\end{equation}
The basis of the tangent space is then $\mathcal{B}_{T_XSO(n)}=\lbrace T^{(k)} = XB^{(k)} \rbrace_{k\in K}$ and to obtain the basis 
...
\end{equation}
The Riemannian metric we define on $T_XSPD(n)$ is given by
\begin{equation}
    \label{eq:spdmetric}
...
on $SPD(n)$ depends also on the base point $X$ of a tangent space $T_XSPD(n)$.
\begin{align}
...
As such, there are different possbilities for choosing equivalence classes and representatives thereof.\\
...
This representation used by Absil et al\cite{AbsilGrassmann} which is very general because only the rank is specified.\\
...
requirement that the basis spanning the subspace $\mathcal{Y}$ must be orthonormal now. The canonical quotient projection map is then given by
\begin{equation}
